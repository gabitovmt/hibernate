...
* _data independence_ - независимость данных.
...
* Java. Идентичность экземпляров (`a == b`). (_object identity_, _объектная идентичность_).
* Java. Равенство экземпляров (`equals()`). (_object quality_, _равенство объектов_).
* SQL. Primary key. (_database identity_, _идентичность в базе данных_).
...
### Понятие сущностей и типов-значений

Есть два человека: John и Jane. Каждая учётная запись представлена экземпляром `User`. У класса `User` есть поле
`homeAddress` - ассоциация с классом `Address`. John и Jane живут в одном доме. Ссылаются оба экземпляра `User`
на один и тот же экземпляр `Address` или у каждого свой экземпляр `Address`?

**Вариант 1**. `User's` ссылаются на один и тот же `Address`.
Здесь `Address` должен поддерживать разделяемую ссылку во время выполнения, это - тип сущности.

**Вариант 2**. `User's` содержат свой `Address`.
Здесь объект `Address` является зависимым от объекта `User`: он становится типом-значением.

Экземпляр _типа сущности_:
* Можно извлечь, используя его хранимую идентичность.
* Свой собственный жизненный цикл.
* Он может существовать независимо от остальных сущностей.

Экземпляр _типа-значения_:
* Отсутствует хранимая идентичность.
* Время жизни определяется экземпляром сущности-владельца.
* Не поддерживает разделения ссылок.

В _JPA_ типы значения называются _базовыми типами_ (_basic property types_) или _встраиваемыми классами_
(_embeddable classes_).

### Отображение сущностей с идентичностью

Если `@Id` расположена над полем класса, _JPA_ будет обращаться напрямую к полям класса и по умолчанию
считать их все частью сохраняемого состояния.

_Candidate key_, _потенциальный ключ_ - это столбец или множество столбцов, которые можно использовать
для идентификации конкретной записи в таблице.
* Значение любого столбца не может быть _null_.
* Значение столбца(-ов) должно быть уникальным.
* Значение столбца(-ов) не изменяемо.

Если обойти неизменяемость первичного ключа, будут проблемы с кэшированием и проверкой состояния объектов _Hibernate_.

_Natural key_, _естественный ключ_ - это ключ, имеющий смысл в предметной области.

_Surrogate key_, _суррогатный ключ_ - это искусственный ключ. **Рекомендуется** использовать его.
